[assembly: Xamarin.Forms.Dependency(typeof(ReactiveUIXF.Services.CacheService))]
namespace ReactiveUIXF.Services
{
    ...
    public class CacheService : ICacheService
    {
        ...
        public async Task<T> GetInMemory<T>(string key)
            => await BlobCache.InMemory.GetObject<T>(key);
        ...
        public async Task<T> GetUserAccount<T>(string key)
            => await BlobCache.UserAccount.GetObject<T>(key);
        ...
        public async Task<T> GetSecure<T>(string key)
            => await BlobCache.Secure.GetObject<T>(key);
        ...
        public async Task<T> GetAndFetch<T>(string key, Func<IObservable<T>> func, DateTimeOffset? expiration)
            => await BlobCache.LocalMachine.GetAndFetchLatest<T>(key, func, null, expiration);

        public async Task<T> GetOrFetch<T>(string key, Func<IObservable<T>> func, DateTimeOffset? expiration)
            => await BlobCache.LocalMachine.GetOrFetchObject<T>(key, func, expiration);
    }
}